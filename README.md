...
2. クラスの作成とインスタンス化
    ...
    - vscodeのターミナルから以下のように打ち込んでディレクトリを作成してください
        ...
        mkdir java01
        ```
    - vscodeのファイルから、ディレクトリを開くで、java01を開く
    - vscodeでctrl+nで新規ファイルを作成、名前を add.javaにする
    - add.javaに以下を打ち込む
        ```
        public class add{
            Integer a;
    ...
    - ここまでのまとめ（クラス、オブジェクト、インスタンス化）
        ...
        - mainクラスは特別で、mainクラスの中にはmainメソッドがあり、プログラムはこのmainメソッドから始まります。このクラスはエントリーポイントとしてプログラム実行時にインスタンス化されメインメソッドが呼び出されます。
    ...
    - 少し修正してみましょう。mainのb.add呼び出しをコメントアウトしてコンパイルし、実行してみてください。結果は次のようになるでしょう
        ```
        c = 0
        a = null, b = null
        ```
...
3. スタティッククラス
    ...
    - スタティッククラスのまとめ
        - インスタンス化せずとも呼び出せる
        - 中身はすべてstatic修飾子をつける必要がある
        - thisは使えない（インスタンス化されていない）のでクラス名ドット〇〇でアクセスする(math.a, math.bの部分)
        - staticクラスは、文字列等のユーティリティ的なクラス（たとえば、和暦西暦変換とか、タイトル文字を作るとか）をまとめておいて、インスタンス化せずとも様々なクラスから使えるようにすると言う使い方をするのが妥当
4. 継承
...
6. インターフェース
    - インターフェースは共通インターフェースといった考え方を実装するためのオブジェクトです。データと実装するべきメソッドを定義しておいてそれを様々なクラスで使いまわすといった使い方をします。
    ...
    - インターフェースのまとめ
        - インターフェースはメソッドやメンバーだけを記述するが、メソッドの具体的な実装は書かない
        - 継承ではなく実装をするのでextendsではなくimplementsを使う
        - 実装であるため、インターフェースに記述しているメソッドは絶対に実装しなければならない
        - （やってないけど）複数のインターフェースを実装できる。（継承では多重継承はできない）
